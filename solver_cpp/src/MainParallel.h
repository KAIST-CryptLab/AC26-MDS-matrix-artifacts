...
public:
  ...
  std::size_t generate_sc(
    ...
    bool incomplete_current_data =
      require_complete_log &&
      fs::is_regular_file(output_path) &&
      !generation_log_exists(output_path);

    if (!force_current) {
      if (known_sc_exists(n, m, c, require_complete_log)) {
        if (!quiet) {
      ...
      if (!force && fs::exists(output_path) && !incomplete_current_data) {
        if (!quiet) {
          std::cout << "GenerateSC(" << n << ", " << m << ", " << c
                    << ") - existing data file could not be read, skip without overwrite\n";
        }
        return 0;
      }
    }
    ...
    if (m == n + 1) {
      generate_sc(n, n, c - 1, false);
      generate_sc(n, m, c - 1, false);
      return run_generation(n, m, c, "OneOutput", [&]() {
        return generate_by_one_output(n, m, c);
      });
    }

    for (int alpha = 2; alpha <= c; ++alpha) {
      generate_sc(n - 1, m, c - alpha, false);
    }
    return run_generation(n, m, c, "OneInput", [&]() {
      return generate_by_one_input(n, m, c);
    });
  }
...
private:
  ...
  TaskSelector prompt_generation_task_selector(
    ...
    while (true) {
      std::cout << generation_name << "(" << n << ", " << m << ", " << c
                << ") enter split task (alpha(>=" << min_alpha
                << ") ranges; example: 2 1-10,15-20,26): "
                << std::flush;
      ...
      if (parts.size() == 2 || parts.size() == 3) {
        try {
          int alpha = std::stoi(parts[0]);
          if (alpha < min_alpha) {
            throw std::runtime_error("alpha is too small");
          }

          if (parts.size() == 2) {
            return TaskSelector{alpha, parse_generation_range_spec(parts[1])};
          }

          int start = parse_positive_int_strict(parts[1]);
          int end = parse_positive_int_strict(parts[2]);
          if (start <= 0 || end < start) {
            throw std::runtime_error("range items must satisfy 1 <= start <= end");
          }
          return TaskSelector{alpha, {{start, end}}};
        } catch (const std::exception&) {
        }
      }

      std::cout << "Invalid input. Use: alpha ranges, with alpha >= "
                << min_alpha << ". Example: 2 1-10,15-20,26\n";
    }
  }
  ...
  bool has_split_source_for_one_input(int n, int m, int c) const {
    for (int alpha = 2; alpha <= c; ++alpha) {
      int previous_n = n - 1;
      int previous_m = m;
      int previous_c = c - alpha;
      if (!sc_source_split_paths(previous_n, previous_m, previous_c).empty()) {
        return true;
      }
    }
    return false;
  ...
  bool has_split_source_for_one_output(int n, int m, int c) const {
    for (int alpha : {0, 1}) {
      int previous_n = n;
      int previous_m = alpha == 0 ? m - 1 : m;
      int previous_c = alpha == 0 ? c - 1 : c - alpha;
      if (!sc_source_split_paths(previous_n, previous_m, previous_c).empty()) {
        return true;
      }
    }
  ...
  void initialize_graph_list_file(const fs::path& path) const {
    ...
    if (!out) {
      throw std::runtime_error("failed to initialize output file: " + path.string());
    }
  }

  void write_generation_log(const fs::path& data_path, std::size_t graph_count, double elapsed_seconds) const {
    fs::path log_path = generation_log_path(data_path);
    std::ofstream out(log_path, std::ios::trunc);
    out << "stored graphs: " << graph_count << "\n";
    out << "time = " << std::fixed << std::setprecision(2) << elapsed_seconds << "s\n";
  }

  void print_generation_summary(std::size_t graph_count, const fs::path& path, double elapsed_seconds) const {
    if (quiet) {
      return;
    }
    std::cout << "stored graphs: " << graph_count << "\n"
              << "path: " << path.string() << "\n"
              << "time = " << std::fixed << std::setprecision(2) << elapsed_seconds << "s\n"
              << "-------------------------\n\n";
  }
  ...
  std::size_t generate_base(int n, int m, int c) {
    ...
    for (auto graph : gen_sc_size_2(c)) {
      graph.reorder(false);
    ...
    print_generation_summary(graphs.size(), output_path, elapsed);
    return graphs.size();
  }
  ...
  std::size_t generate_by_one_input(int n, int m, int c) {
    ...
    if (split_output) {
      ...
      auto selector = prompt_generation_task_selector(n, m, c, "GenerateSCByOneInput", 2);
    ...
    for (const auto& unit : units) {
      ...
      std::ofstream out(unit.output_path, std::ios::binary | std::ios::app);
      std::vector<std::string> output_buffer;
      auto worker = [alpha = unit.alpha](Graph previous_graph) {
        return validate_generated_graphs(
          expand_sc_by_one_input(previous_graph, alpha),
          SuperconcentratorCheckMode::NewInput
        );
      };
      run_parallel_source_graphs(
        split_output ? unit.output_path.filename().string() : "alpha=" + std::to_string(unit.alpha),
        previous_graphs,
        worker,
        hashes,
        out,
  ...
  std::size_t generate_by_one_output(int n, int m, int c) {
    ...
    if (split_output) {
      ...
      auto selector = prompt_generation_task_selector(n, m, c, "GenerateSCByOneOutput", 0);
      units = select_generation_units(pending_units, selector);
    ...
    for (const auto& unit : units) {
      ...
      auto worker = [alpha = unit.alpha](Graph previous_graph) {
        if (alpha > 0) {
          return validate_generated_graphs(
            expand_sc_by_internal_nodes(previous_graph, alpha),
            SuperconcentratorCheckMode::Assume
          );
        }
        return validate_generated_graphs(
