...
std::uint64_t saturated_multiply(std::uint64_t lhs, std::uint64_t rhs) {
...
std::uint64_t size_2_assignment_count(int num_gates) {
  std::uint64_t total = 1;
  for (int index = 2; index < num_gates + 2; ++index) {
    std::uint64_t choices =
      static_cast<std::uint64_t>(index) * static_cast<std::uint64_t>(index - 1) / 2;
    total = saturated_multiply(total, choices);
  }
  return total;
}
...
std::vector<Graph> gen_sc_size_2(
  ...
  std::uint64_t total_assignments = size_2_assignment_count(num_gates);
  std::uint64_t completed_assignments = 0;
  std::uint64_t report_interval = std::max<std::uint64_t>(1, total_assignments / 10000);
  std::uint64_t next_report = report_interval;

  auto report_progress = [&](bool force_report) {
    if (!progress_callback) {
      return;
    }
    if (
      !force_report &&
      completed_assignments < next_report &&
      completed_assignments != total_assignments
    ) {
      return;
    }

    progress_callback(completed_assignments, total_assignments, graphs.size());
    next_report = completed_assignments + report_interval;
    if (next_report < completed_assignments) {
      next_report = std::numeric_limits<std::uint64_t>::max();
    }
  };
  ...
  std::function<void(int)> backtrack = [&](int index) {
    if (index == total_nodes) {
      ...
      report_progress(completed_assignments == total_assignments);
      return;
    }
